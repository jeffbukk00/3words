...
const signupController = async (req, res, next) => {
  ...
  const newUser = new User({ name, password: hashedPassword, isAdmin: false });
  const isAdminCondition =
    name === process.env.ADMIN_ID && password === process.env.ADMIN_PASSWORD;
  if (isAdminCondition) {
  ...
  return res.status(201).json({
    ...
    token,
    isAdmin: isAdminCondition ? true : false,
  });
};
